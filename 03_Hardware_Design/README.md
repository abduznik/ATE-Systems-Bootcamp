...
2. **מימוש המעגל:** עליכם לממש **מגבר שרת בתצורת מגבר לא-הופך (Non-Inverting Amplifier)**.
3. **דרישות מתח והגבר:**
   - מתח ההזנה (VCC) של המעגל הוא **5V**.
   - המעגל יספק **הגבר (Gain) של 2**.
   - מתח המוצא המקסימלי ל-ESP32 חייב להיות **3.3V** (ודאו שמתח הכניסה תואם ליחס ההגבר).
   - למימוש ההגבר יש להשתמש בשני נגדי **10kΩ**.
